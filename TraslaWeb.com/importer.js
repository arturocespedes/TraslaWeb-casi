...
let keywordsPermitidos = ["Buenas prácticas", "Tras la mentira", "Tras la interrupción", "Tras la confusión", "Tras los pagos"  ]; // Ajusta estas keywords según sea necesario
...
function constructSelector() {
  ...
  sel.option("Selecciona un tipo de patrón ");
...
function handleScroll() {
  ...
  casos.forEach(function(caso) {
    // Verificar si el caso está en la pantalla
    if (isElementInViewport(caso)) {
      // Si está en la pantalla, aplicar estilos de iluminación
      caso.style.opacity = '1';
      caso.style.boxShadow = '0 0 30px rgba(238, 207, 163, 1)'; // Ajusta el color y la intensidad de la sombra según sea necesario
      
    } else {
      // Si no está en la pantalla, restablecer los estilos
      caso.style.opacity = '0.5';
      caso.style.boxShadow = 'none';
...
// También puedes llamar a handleScroll() inicialmente para aplicar estilos a los casos visibles al cargar la página
handleScroll();
